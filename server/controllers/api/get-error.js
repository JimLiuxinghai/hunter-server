...
const mapDir = path.join(__filename, '../../../../', 'source_map/ucenter');
...
function lookupSourceMap(mapFile, line, column, callback) {
    fs.readFile(mapFile, async (err, data) => {
        ...
        //console.log(fileContent, fileObj, sources, '&&&&&&&')
        sources.map(item => {
        ...
        consumer.then((result) => {
            var result = result.originalPositionFor(lookup);
...
export default {
    ...
    */
     ...
     * @param {*} ctx 
     */
    async getErrDetail (ctx) {

        let msg = ctx.query.msg;
        //console.log(msg, 'msg')
        var objArr = msg.split('at');
        var nameInfo = path.basename(objArr[1]).replace(')', '').split(':');
        
        let [filename, line, col] = nameInfo;
        console.log(nameInfo, 'name')
        let filePath = fs.readFileSync(path.join(mapDir, (filename + ".map")), 'utf8')
        let smc = new sourceMap.SourceMapConsumer(filePath);
        smc.then(function(result) {

            let ret = result.originalPositionFor({
                line: parseInt(line, 10), // 压缩后的行号
                column: parseInt(col, 10) // 压缩后的列号
            });
            let data = Tips.ERR_OK;
            data.data = ret;
            ctx.body = data;
        })
        // lookupSourceMap(path.join(mapDir, (filename + ".map")), line, col, function (res) {
